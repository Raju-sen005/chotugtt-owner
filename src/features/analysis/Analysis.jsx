  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-10 space-y-8 font-sans bg-[#F8F9FA] min-h-screen">
      ...
      <div className="flex items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Business Analytics</h2>
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">Track revenue metrics, sales trends, and table performance.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xs space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="font-black text-slate-900 text-base">Weekly Sales Trend</h3>
            <span className="text-[10px] font-bold uppercase tracking-wider bg-slate-100 text-slate-600 px-3 py-1 rounded-full">Last 7 Days</span>
          </div>
          <div className="pt-4">
            <ResponsiveContainer width="1000%" height={240} className="w-full !max-w-full">
              <BarChart data={chartData}>
                ...
                <Bar dataKey="sales" fill="#f43f5e" radius={[6, 6, 0, 0]} barSize={36} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          {Object.entries(hourlyData).map(([label, val]) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs font-medium text-slate-400 mt-0.5">orders placed</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <h3 className="font-black text-slate-900 text-base">Table Performance</h3>
            <span className="text-xs font-bold text-slate-500">{sortedTableStats.length} Tables Active</span>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {displayTables.length > 0 ? (
                  displayTables.map((t) => {
                    ...
                    return (
                      <tr key={t._id} className="group hover:bg-slate-50/80 transition-colors">
                        <td className="py-4 font-bold text-slate-800">Table {t._id || "N/A"}</td>
                        <td className="py-4 text-right">
                          <div className="flex items-center justify-end gap-4">
                            <span className="font-black text-red-500">{t.orderCount}</span>
                            <div className="w-20 h-2 bg-slate-100 rounded-full overflow-hidden">
                              <div className="h-full bg-gradient-to-r from-red-400 to-rose-500 rounded-full" style={{ width: `${percentage}%` }} />
                            </div>
                          </div>
                        </td>
                      </tr>
                  ...
                  <tr>
                    <td colSpan="2" className="py-12 text-center text-slate-400 italic text-xs">
          ...
          {sortedTableStats.length > 5 && (
            ...
            >
              {showAllTables ? "Show Less Tables" : `View All (${sortedTableStats.length}) Tables`}
            </button>
          )}
        </div>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/80 shadow-xs space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100">
            <h3 className="font-black text-slate-900 text-base">Top Selling Items</h3>
            <span className="text-xs font-bold text-slate-500">{topItems.length} Featured Items</span>
          </div>
                ...
                {topItems.length > 0 ? (
                  topItems.map((item, i) => (
                    <tr key={i} className="group hover:bg-slate-50/80 transition-colors">
                      <td className="py-4 font-bold text-slate-800">{item._id}</td>
                      <td className="py-4 text-right">
